...
All models used the following components to model the remaining mass of litterbag replicate $i$ conditional on the average remaining mass ($\mu_i$) , the precision of remaining masses ($\phi_i$), and decomposition rates ($k\_1_i$) (equation \@ref(eq:sup-model-1)):
...
The formula for the average remaining mass ($\mu_i$) when $\alpha=1$ and there are no initial leaching losses (models 2-1 and 2-2), according to equation \@ref(eq:decomposition-solution-1-no-leaching-1) in the main text, are (equation \@ref(eq:sup-model-2)):
...
\end{cases}\\
s & = & `r mass_relative_mass_offset`\\
(\#eq:sup-model-2)
\end{aligned}
\end{equation}

The formula for the average remaining mass ($\mu_i$) when $\alpha=1$ and there are initial leaching losses (models 1-1 and 1-2), according to equation \@ref(eq:decomposition-solution-1-with-leaching-1) in the main text, are (equation \@ref(eq:sup-model-3)):
...
The formula for the average remaining mass ($\mu_i$) when $\alpha$ is estimated from the litterbag data and there are no initial leaching losses (models 2-3 and 2-4), according to equation \@ref(eq:decomposition-solution-2-no-leaching-1) in the main text, are (equation \@ref(eq:sup-model-4)):
...
The formula for the average remaining mass ($\mu_i$) when $\alpha$ is estimated from the litterbag data and there are initial leaching losses (models 1-3, 1-4), according to equation \@ref(eq:decomposition-solution-2-with-leaching-1) in the main text, are (equation \@ref(eq:sup-model-5)):
...
$\alpha$ is modeled in the same way as $\phi$ (models 1-3, 1-4, 2-3, 2-4) (equation \@ref(eq:sup-model-6)):
...
Initial leaching losses are modeled in the same way as $\phi$ (models 1-3, 1-4) (equation \@ref(eq:sup-model-7)):
...
When equation \@ref(eq:decomposition-solution-2-with-leaching-1) in the main text is used to estimate one-pool decomposition rates from litterbag experiments with large initial mass losses as caused by initial leaching, estimates for $k_0$ and $\alpha$ are much larger, indicating that under these conditions parameters intended to describe how decomposition rates decrease over time incorporate the effect of mass losses due to initial leaching. Fig. S\@ref(fig:sup-out-mm35-1-p5) shows estimates for decomposition rates and $\alpha$ for the available litterbag data and Fig. S\@ref(fig:sup-out-mm35-2-p5) shows the same when data from @Bengtsson.2017 are excuded.
...
(ref:sup-out-p-sdm-all-models-check-3-1-caption) Density estimate of 100 sets of remaining mass errors (converted to precision) sampled from the prior distribution of each model (light blue lines) versus density estimate of the measured remaining masses from the litterbag studies.
...
(ref:sup-out-p-sdm-all-models-check-4-1-caption) Density estimate of 100 sets of remaining mass errors (converted to precision) sampled from the posterior distribution of each model (light blue lines) versus density estimate of the measured remaining masses from the litterbag studies.
...
<!--However, a direct transfer of the results of this sensitivity analysis to the available litterbag data is not possible because unlike our simulation, the design of litterbag experiments, and specific decomposition rates and initial leaching losses for experiments and species, are different. In addition, in hierarchical models, the amount by which $l_0$ and $k_0$ estimates are pooled to the (studies', species', and global) averages depends on how much information the data provide for individual experiments and how much the true values vary in the hierarchical groups, and these factors are different between model 1-5 and available litterbag data.  -->
...
(ref:sup-out-sdm-all-models-p1-caption) Measured versus predicted remaining masses in litterbag studies from all models computed in our study. Points are average values and error bars are 95% prediction intervals. For a description of each model, see Tab. S\@ref(tab:m-litterbag-synthesis-models).
