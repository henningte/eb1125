...
(ref:eq-hpm-hpm-l-2) \@ref(main-eq:hpm-hpm-l-2)
(ref:eq-hpm-modified-granberg-1) \@ref(main-eq:hpm-modified-granberg-1)
(ref:eq-model-link-2) \@ref(main-eq:model-link-2)
(ref:eq-model-link-1) \@ref(main-eq:model-link-1)
(ref:eq-hpm-moisture-modifier-2) \@ref(main-eq:hpm-moisture-modifier-2)
(ref:eq-hpm-moisture-modifier-1) \@ref(main-eq:hpm-moisture-modifier-1)
(ref:eq-hpm-decomposition-rate-1) \@ref(main-eq:hpm-decomposition-rate-1)
...
hpmd_tables$prior_justification %>%
  ...
  kableExtra::column_spec(6, width = "7cm")
```
...
(ref:sup-hpmd-plot-ppc-prior-hpm-l-2-caption) Density estimate of 100 sets of initial leaching losses ($l_0$) predicted by `r hpmd_model_id_to_name_2("3-4")` sampled from the prior distribution (light blue lines) versus density estimate of the initial leaching loss estimated from the litterbag studies.
...
(ref:sup-hpmd-plot-ppc-posterior-hpm-l-2-caption) Density estimate of 100 sets of initial leaching losses ($l_0$) predicted by `r hpmd_model_id_to_name_2("3-4")` sampled from the posterior distribution (light blue lines) versus density estimate of the initial leaching loss estimated from the litterbag studies.
...
# Marginal posterior distributions of HPM parameters in `r hpmd_model_id_to_name_2(c("3-3", "3-4")) |> knitr::combine_words(and = " and ")` {#sup-5}

(ref:sup-hpmd-plot-4-3-caption) Marginal posterior distributions of HPM decomposition module parameters as estimated by `r hpmd_model_id_to_name_2("3-3")`. (a) Marginal posterior distributions for $c_1$, $W_{opt}$, $f_{min}$, and $c_2$ (see Tab. \@ref(main-tab:hpmd-m-tab-hpm-parameters-standard-values) in the main text for details). Vertical black lines are the standard parameter values from @Frolking.2010. (b) Marginal posterior distributions for $k_0$ estimated for each species. Species were assigned to HPM microhabitats as described in the Methods section in the main text. *Sphagnum* spec. are samples which have been identified only to the genus level.

```{r sup-hpmd-plot-4-3, fig.cap='(ref:sup-hpmd-plot-4-3-caption)', out.width="100%"}
knitr::include_graphics(tar_read(hpmd_plot_4_3))
```

(ref:sup-hpmd-plot-4-4-caption) Marginal posterior distributions of HPM decomposition module parameters as estimated by `r hpmd_model_id_to_name_2("3-4")`. (a) Marginal posterior distributions for $c_1$, $W_{opt}$, $f_{min}$, and $c_2$ (see Tab. \@ref(main-tab:hpmd-m-tab-hpm-parameters-standard-values) in the main text for details). Vertical black lines are the standard parameter values from @Frolking.2010. (b) Marginal posterior distributions for $k_0$ estimated for each species. Species were assigned to HPM microhabitats as described in the Methods section in the main text. *Sphagnum* spec. are samples which have been identified only to the genus level.
...
(ref:sup-hpmd-plot-5-caption) Marginal posterior distributions of HPM decomposition module parameters as estimated by `r hpmd_model_id_to_name_2("3-4")` during the cross-validation. During the cross-validation, one of the cross-validation folds was left out each time and the model was refitted, producing a marginal posterior distribution for each parameter and cross-validation block. (a) Marginal posterior distributions for $c_1$, $W_{opt}$, $f_{min}$, and $c_2$ (see Tab. \@ref(main-tab:hpmd-m-tab-hpm-parameters-standard-values) in the main text for details). (b) Marginal posterior distributions for $k_0$ estimated for each species for which data were removed during the cross-validation.
...
# Depth profiles of predicted decomposition rates with $W_{opt}$ estimated by `r hpmd_model_id_to_name_2("3-4")` or set to its standard value for *S. fallax* {#sup-9}

(ref:sup-hpmd-simulation-1-plot-5-caption) Decomposition rates predicted with `r hpmd_model_id_to_name_2("3-4")` ($k_{0,\text{modified}}(\text{`r hpmd_model_id_to_name_2("3-4")`})$) for *S. fallax* (hollows), using either the standard value for $W_{opt}$ or the $W_{opt}$ value estimated by `r hpmd_model_id_to_name_2("3-4")` versus depth of the litter below the peat surface. The horizontal line is the average water table depth.
...
# Prediction uncertainties of `r hpmd_model_id_to_name_2("3-4")` {#sup-11}
...
# R code to predict $k_0$, $l_0$, and remaining masses with `r hpmd_model_id_to_name_2("3-4")` {#sup-7}

`r hpmd_model_id_to_name_2("3-4")` and functions to predict $k_0$ and $l_0$ for different species and water table levels are available via the R package hpmdpredict [@Teickner.2025b]. To make predictions, one first has to define some variables like the incubation duration. Here, we predict remaining masses and initial leaching losses for *S. fuscum* incubated at a degree of saturation of 0.6 L$_\text{water}$ L$_\text{pores}^{-1}$ during the first five years.
